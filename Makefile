.PHONY: smoke test-critical test-all lint ruff test-drift install-hooks help

help: ## Show this help
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | awk 'BEGIN {FS = ":.*?## "}; {printf "  %-18s %s\n", $$1, $$2}'

smoke: ## Run the 6 drift-guarded smoke tests (~15s) — same as pre-commit
	cd backend && python -m pytest \
		tests/test_admin_users_multicurrency_display.py \
		tests/test_iter27_auth_refactor.py::test_openapi_path_count_unchanged \
		tests/test_multicurrency_and_stats.py::TestVipBalancesEndpoint::test_vip_legacy_plus_dict_usdt_conversion \
		tests/test_p2p_backend.py::TestUsersAdmin::test_list_and_update_user \
		tests/test_p2p_backend.py::TestOrders::test_orders_mine_isolation \
		tests/test_iter55_19g_notification_explorer_link.py \
		-q

test-critical: ## Run critical regression subset (~2 min, 190 tests) — pre-commit safety net
	cd backend && python -m pytest \
		tests/test_iter55_16_permissions.py \
		tests/test_iter55_16b_audit_perm_snapshot.py \
		tests/test_company_fund_adjustments.py \
		tests/test_company_funds_role_split.py \
		tests/test_iter55_18_delete_notifications.py \
		tests/test_iter55_19c_crypto_network_validation.py \
		tests/test_iter55_19h_tx_hash_network_validation.py \
		tests/test_iter14_corrections.py \
		tests/test_totp_2fa.py \
		tests/test_iter55_37_session_regression.py \
		tests/test_iter55_36m_defensive_mode_toggle.py \
		tests/test_iter55_36o_verification_gate.py \
		tests/test_iter55_36q_bulk_approve_kyc.py \
		tests/test_orphan_field_audit.py \
		tests/test_language_preference.py \
		tests/test_localized_notifications.py \
		tests/test_iter247_negative_amounts.py \
		tests/test_iter248_race_conditions.py \
		tests/test_iter249_credit_recovery.py \
		tests/test_iter250_validate_before_debit.py \
		tests/test_iter252_applied_templates.py \
		tests/test_iter254_audit_fixes.py \
		tests/test_iter256_revision_fixes.py \
		tests/test_iter257_deep_audit.py \
		tests/test_iter260_continuation_audit.py \
		tests/test_iter261_audit_f_fixes.py \
		tests/test_iter262_audit_g_fixes.py \
		tests/test_iter263_cashbox_mirror.py \
		tests/test_iter264_arqueo_programado.py \
		tests/test_iter265_audit_caja_fixes.py \
		tests/test_iter268_client_balance_breakdown.py \
		tests/test_iter269_verificacion_caja_fixes.py \
		tests/test_iter270_revision_flujo_caja.py \
		tests/test_iter271_flujo_caja_m01_m06.py \
		tests/test_iter272_p01_p03.py \
		tests/test_iter273_q01_q02.py \
		tests/test_iter274_r01.py \
		tests/test_iter277_denoms_sync.py \
		tests/test_iter279_s01_s07.py \
		tests/test_iter281_t01_t04.py \
		tests/test_iter282_u01.py \
		tests/test_iter283_audit_lotes.py \
		tests/test_iter284_revision_r01_r06.py \
		tests/test_iter285_pay_now_marketplace.py \
		tests/test_iter286_sell_rates_receipt.py \
		tests/test_iter287_audit_v01_v03.py \
		tests/test_iter288_audit_mensajeria.py \
		tests/test_iter289_fase_a_mejoras.py \
		tests/test_iter290_fase_b_mejoras.py \
		tests/test_iter291_w01_w02_fase_c.py \
		tests/test_iter292_audit_monedas.py \
		tests/test_iter293_revision_mensajeria.py \
		tests/test_iter294_verificacion_fx.py \
		tests/test_iter295_ms01_ms04.py \
		tests/test_iter296_rt01_rt02.py \
		tests/test_iter297_mv01_mv02.py \
		tests/test_iter298_conciliacion.py \
		tests/test_iter299_depositos_retiros.py \
		-q --tb=line

test-all: ## Run the full pytest suite (~8-9 min, 935+ tests)
	cd backend && python -m pytest tests/ -q --tb=line

lint: ## Frontend eslint (react-hooks + syntax)
	cd frontend && yarn lint

ruff: ## Backend ruff — undefined names + literal comparisons
	python -m ruff check --select F821,F822,F632 backend/

test-drift: smoke ## Alias for `smoke`

install-hooks: ## Wire up .git/hooks/pre-commit (one-time per clone)
	git config core.hooksPath .githooks
	@echo "✓ Git hooks pointing to .githooks/. Every commit now runs the secret-scan + critical tests."
