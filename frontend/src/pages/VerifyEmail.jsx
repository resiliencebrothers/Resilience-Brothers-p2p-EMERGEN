  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] text-white p-4">
      <div className="tactile-card p-10 max-w-md w-full text-center" data-testid="verify-email-page">
        {state === "loading" && <><Loader2 className="w-10 h-10 text-[#8B5CF6] animate-spin mx-auto mb-4" /><p className="text-neutral-400">Verificando tu correo...</p></>}
        ...
        {state === "err" && <><AlertTriangle className="w-12 h-12 text-[#EF4444] mx-auto mb-4" data-testid="verify-email-error-icon" /><h1 className="font-display text-2xl mb-2" data-testid="verify-email-error-title">No se pudo verificar</h1><p className="text-neutral-400 text-sm mb-6" data-testid="verify-email-error-msg">{msg}</p><Link to="/" className="text-[#8B5CF6] hover:underline text-sm" data-testid="verify-email-back-link">← Volver al inicio</Link></>}
      </div>
    </div>
  );
}
