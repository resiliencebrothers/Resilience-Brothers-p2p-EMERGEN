 ...
 * so the language follows them across devices.
 */
  ...
  return (
    <button
      ...
      onClick={() => {
        ...
        syncLanguagePreferenceToServer(nextCode);
      }}
      data-testid={testid}
      aria-label={`Switch language to ${nextCode.toUpperCase()}`}
      className="inline-flex items-center gap-1.5 px-2.5 h-9 rounded-full border border-white/15 hover:border-violet-400/60 hover:bg-white/5 text-white/70 hover:text-white text-xs font-medium transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-violet-500 whitespace-nowrap shrink-0"
    >
