...
### P1
- Refactor Phase 3: extract `routes/orders.py`, `routes/admin.py`, `routes/market.py`, `routes/products.py` from `server.py` (still 2637 lines).
- Move `_enforce_totp_step_up` from `server.py` to `auth_utils.py` to eliminate the lazy import in `routes/blocklist.py` verify/reject-phone endpoints (last server↔routes coupling).
