...
def generate_transactions_pdf(entries: list, filters: dict, totals: dict) -> bytes:
    ...
    story = []
    story.append(Paragraph("/ REGISTRO CONTABLE", label))
    story.append(Paragraph("Transacciones — Resilience Brothers", h1))
    ...
    for code in sorted(by_cur.keys()):
        ...
        totals_lines.append(
            f"<font color='#EAB308'><b>{code}</b></font>: "
    ...
    if totals_lines:
        story.append(Paragraph(
            "<b>TOTALES POR MONEDA</b><br/>" + "<br/>".join(totals_lines),
            totals_style
        ))
    story.append(Paragraph(
        f"Total de transacciones: <font color='#EAB308'><b>{len(entries)}</b></font>",
        sub,
    ))
    ...
    # Color column Tipo by direction
